...
type SidebarModalsProps = {
  ...
  bulkDeleteConfirmation: { count: number } | null;
  isBulkDeleteRunning: boolean;
  onCancelBulkDelete: () => void;
  onConfirmBulkDelete: (hardDelete?: boolean) => void;
            ...
            <div className="w-full max-w-md overflow-hidden rounded-xl border border-border bg-card shadow-2xl">
              <div className="p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="mb-2 text-lg font-semibold text-foreground">
                      {t('deleteConfirmation.bulkDeleteSessions', 'Delete {{count}} sessions?', { count: bulkDeleteConfirmation.count })}
                    </h3>
                    <p className="mb-1 text-sm text-muted-foreground">
                      {t('deleteConfirmation.bulkSessionsSelected', '{{count}} conversations selected.', { count: bulkDeleteConfirmation.count })}
                    </p>
                    <p className="mt-3 text-xs text-muted-foreground">
                      {t('deleteConfirmation.bulkSessionsNotice', 'Archiving hides the sessions while keeping their history; deleting permanently removes the session records and their transcript files.')}
                    </p>
                  </div>
                </div>
              </div>
              <div className="flex flex-col gap-2 border-t border-border bg-muted/30 p-4">
                <Button
                  ...
                  onClick={() => onConfirmBulkDelete(false)}
                >
                  <EyeOff className="mr-2 h-4 w-4" />
                  {t('deleteConfirmation.bulkArchiveSessions', 'Archive {{count}} sessions', { count: bulkDeleteConfirmation.count })}
                </Button>
                <Button
                  variant="destructive"
                  className="w-full justify-start bg-red-600 text-white hover:bg-red-700"
                  disabled={isBulkDeleteRunning}
                  onClick={() => onConfirmBulkDelete(true)}
                >
                  <Trash2 className="mr-2 h-4 w-4" />
                  {isBulkDeleteRunning
                    ? t('recent.bulkDeleting', 'Deleting...')
                    : t('deleteConfirmation.bulkDeleteSessionsPermanently', 'Delete {{count}} sessions permanently', { count: bulkDeleteConfirmation.count })}
                </Button>
                <Button variant="ghost" className="w-full" onClick={onCancelBulkDelete} disabled={isBulkDeleteRunning}>
                  {t('actions.cancel')}
                </Button>
              </div>
            </div>
