...
export function useSidebarController({
  ...
  const [bulkSelectedSessionIds, setBulkSelectedSessionIds] = useState<ReadonlySet<string>>(new Set());
  ...
  // Optimistically flip a session's pinned flag in the recent-conversations
  // list, persist via the API, and reload to keep ordering (pinned first).
  const handleTogglePin = useCallback(async (sessionId: string, isPinned: boolean) => {
    ...
    try {
      const response = await api.pinSession(sessionId, isPinned);
      if (!response.ok) {
        throw new Error(`Failed to pin session: ${response.status}`);
      }
      void fetchRecentConversationsPage(0, false);
    } catch (error) {
  ...
  const toggleBulkSessionSelected = useCallback((sessionId: string) => {
    setBulkSelectedSessionIds((previous) => {
      const next = new Set(previous);
  ...
  const bulkSetSelectedSessions = useCallback((sessionIds: string[], selected: boolean) => {
    setBulkSelectedSessionIds((previous) => {
      const next = new Set(previous);
  ...
  const requestBulkDeleteConfirmation = useCallback(() => {
    if (bulkSelectedSessionIds.size === 0) {
      return;
    }
    setBulkDeleteConfirmation({ count: bulkSelectedSessionIds.size });
  }, [bulkSelectedSessionIds]);

  const confirmBulkDeleteSessions = useCallback(async (hardDelete = false) => {
    const sessionIds = [...bulkSelectedSessionIds];
    ...
    try {
      const response = await api.deleteSessionsBatch(sessionIds, hardDelete);
      if (response.ok) {
        ...
        const errorText = await response.text();
        console.error('[Sidebar] Failed to batch delete sessions:', {
          status: response.status,
          error: errorText,
        });
        alert(t('messages.deleteSessionFailed'));
      ...
      console.error('[Sidebar] Error batch deleting sessions:', error);
      alert(t('messages.deleteSessionError'));
    } finally {
      setIsBulkDeleteRunning(false);
    }
  }, [bulkSelectedSessionIds, exitBulkSelectionMode, fetchArchivedSessions, fetchRecentConversationsPage, onSessionDelete, t]);
  ...
  const updateSessionSummary = useCallback(
    ...
    [fetchRecentConversationsPage, onRefresh, t],
  );
  ...
  return {
    ...
    exitBulkSelectionMode,
    toggleBulkSessionSelected,
    bulkSetSelectedSessions,
