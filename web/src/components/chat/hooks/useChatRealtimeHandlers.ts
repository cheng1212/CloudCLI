    ...
    const handleEvent = (msg: ServerEvent) => {
      ...
      switch (msg.kind) {
        ...
        case 'status': {
          ...
          } else if (msg.text === 'model_routed' && sid) {
            // Server-side route resolution: record the model this run actually
            // talks to (the endpoint truth), so the activity indicator can show
            // it instead of the catalog model id. `model` may be absent when a
            // pre-defined model keeps the CLI's own endpoint.
            onSessionProcessing?.(sid, {
