...
export function useChatProviderState({ selectedSession, selectedProject: _selectedProject }: UseChatProviderStateArgs) {
  ...
  const selectProviderModel = useCallback(async (
    ...
    // 切换即用：同步乐观更新 session 级模型，不等 POST 返回。否则切换后
    // 立即发送的消息会带着旧的 sessionModel（currentProviderModel 优先取
    // sessionModel），造成"切了但发的还是旧模型"。POST 结果一致，成功后
    // 再用服务端返回值覆盖；失败则回滚到切换前的选择。
    setSessionSelection({
      provider: targetProvider,
  ...
  const setHiddenCustomModel = useCallback(async (
    targetProvider: LLMProvider,
    existing: ProviderModelOption,
    hidden: boolean,
  ) => {
    if (!existing.recordId) {
      throw new Error('This model cannot be hidden.');
    }

    const response = await authenticatedFetch(
      `/api/providers/${targetProvider}/models/${existing.recordId}/hidden`,
      {
        method: 'PATCH',
    ...
    // A hidden active model must not stay selected: fall back to the catalog
    // default the same way deletion does.
    if (hidden && providerModels[targetProvider] === existing.value) {
      setStoredProviderModel(targetProvider, result.models.DEFAULT);
    }
    if (hidden && provider === targetProvider && sessionModel === existing.value) {
      setSessionSelection((current) => current ? {
        ...current,
        model: result.models.DEFAULT,
      } : current);
  ...
  const providerModelActions = useMemo<ProviderModelActions>(() => ({
    ...
    setHidden: setHiddenCustomModel,
  }), [createCustomModel, removeCustomModel, setHiddenCustomModel, updateCustomModel]);
