...
export function useChatComposerState({
  ...
  // Guards the send button against rapid re-clicks while an attachment upload
  // is still in flight. `isSubmitting` disables the button; the ref is the
  // synchronous re-entrancy backstop so a second submit in that window is
  // dropped instead of re-uploading/racing the first one.
  ...
  // Dedicated hidden file inputs for the attach menu. react-dropzone's `open()`
  // cannot scope the picker to image files, so the composer renders two inputs
  // (generic + image/*) and these methods trigger the right one. On mobile the
  // image input with accept="image/*" opens the system photo gallery.
  ...
  const handleSubmit = useCallback(
    async (
      ...
      // A rapid second click while an image is still uploading should not
      // re-submit; the send button is also disabled for this window, and the
      // ref below is the synchronous backstop against re-entrancy.
