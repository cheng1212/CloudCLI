...
function ChatInterface({
  ...
  // Plan progress for the active session, derived from its latest TodoWrite
  // tool call. Memoized on the merged transcript reference, so realtime todo
  // updates re-derive on the same store ticks that re-render chat.
  const planSessionId = selectedSession?.id || currentSessionId || null;
  const sessionTodos = useSessionTodos(sessionStore, planSessionId);
