...
type ProviderGroup = {
  ...
  /** Source-grouped buckets for two-level rendering; null = flat list. */
  modelBuckets: ReturnType<typeof groupModelOptions> | null;
};
...
export default function ProviderSelectionEmptyState({
  ...
  // Cascading source rows: which "provider::group" is drilled into level 2.
  const [openSourceKey, setOpenSourceKey] = useState<string | null>(null);
  // A non-empty search switches grouped catalogs back to a flat filtered list.
  ...
  const visibleProviderGroups = useMemo<ProviderGroup[]>(() => {
    return PROVIDER_META.map((p) => {
      ...
      const buckets = groupModelOptions(models);
      return {
        id: p.id,
        name: p.name,
        models,
        // Null when the catalog has no source groups → flat rendering.
        modelBuckets: hasModelGroups(buckets) ? buckets : null,
      };
    });
  }, [providerModelCatalog]);
                ...
                <CommandList className="max-h-[350px]">
                  ...
                  {visibleProviderGroups.map((group, idx) => (
                    ...
                    >
                      ...
                      {(() => {
                        const renderModel = (model: ProviderModelOption) => {
                          const isSelected = provider === group.id && currentModel === model.value;
                          return (
                            <CommandItem
                              key={`${group.id}-${model.value}`}
                              value={`${group.name} ${model.group || ''} ${model.label} ${model.description || ''}`}
                        ...
                        if (!group.modelBuckets || modelSearch.trim() !== "") {
                          // No source groups, or the user is searching: flat list.
                          return group.models.map(renderModel);
                        }

                        const groupSourceKey = (source: string) => `${group.id}::${source}`;
                        const openBucket = group.modelBuckets.find(
                          (bucket) => openSourceKey === groupSourceKey(bucket.group || ""),
                        );

                        if (openBucket) {
                          // Level 2: the drilled-in source's models + back row.
                          return (
                            <>
                              <CommandItem
                                value={`back ${group.name} ${openBucket.group || ""}`}
                                onSelect={() => setOpenSourceKey(null)}
                                className="ml-4 border-l border-border/40 pl-4 text-muted-foreground"
                              >
                                <ChevronLeft className="mr-2 h-3.5 w-3.5 shrink-0" />
                                <span className="truncate">
                                  {openBucket.group || t("providerSelection.otherModels", { defaultValue: "Other models" })}
                                </span>
                              </CommandItem>
                              {openBucket.options.map(renderModel)}
                            </>
                          );
                        }

                        // Level 1: source rows, tap to drill in.
                        return group.modelBuckets.map((bucket) => {
                          const sourceKey = groupSourceKey(bucket.group || "");
                          const hasSelectedModel = bucket.options.some(
                            (model) => provider === group.id && currentModel === model.value,
                          );
                          return (
                            <CommandItem
                              key={`source-${group.id}-${bucket.group || "ungrouped"}`}
                              value={`${group.name} ${bucket.group || ""}`}
                              onSelect={() => setOpenSourceKey(sourceKey)}
                              className="ml-4 border-l border-border/40 pl-4"
                            >
                              <div className="min-w-0 flex-1">
                                <span className="truncate">
                                  {bucket.group || t("providerSelection.otherModels", { defaultValue: "Other models" })}
                                </span>
                              </div>
                              {hasSelectedModel && (
