...
export interface SessionActivity {
  ...
  startedAt: number;
  /**
   * The model this run actually talks to, as resolved by the server route
   * (the endpoint truth, not the catalog model id). Populated by the
   * `model_routed` status event and shown in the activity indicator.
   */
  model?: string | null;
}
...
export type MarkSessionProcessing = (
  ...
  activity?: { statusText?: string | null; canInterrupt?: boolean; model?: string | null },
) => void;
...
const sessionActivityMapsMatch = (
  ...
  for (const [sessionId, leftActivity] of left) {
    ...
    if (
      ...
      || leftActivity.model !== rightActivity.model
    ) {
      return false;
    }
  }
  ...
  const markSessionProcessing = useCallback<MarkSessionProcessing>((sessionId, activity) => {
    ...
    setProcessingSessions((prev) => {
      ...
      const next: SessionActivity = {
        ...
        model:
          activity?.model !== undefined ? activity.model : existing?.model ?? null,
      };
    ...
    setProcessingSessions((prev) => {
      ...
      for (const [sessionId, snapshot] of incoming) {
        ...
        updated.set(sessionId, {
          ...
          model: snapshot.model !== undefined ? snapshot.model : existing?.model ?? null,
        });
      }
