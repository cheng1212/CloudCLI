...
import { clearSessionPageCache } from '../stores/sessionMessageCache';
...
// ─── Project-list display cache (fast PWA cold start) ────────────────────────

const PROJECTS_CACHE_KEY = 'cloudcli.projectsCache.v1';

/**
 * Sidebar snapshot from the last successful fetch, persisted so a cold start
 * renders the session list immediately while `fetchProjects` revalidates in
 * the background. TaskMaster payloads are stripped before saving — they are
 * large, re-hydrated per selected project, and only cached in memory.
 */
...
export function useProjectsState({
  ...
  // Persist the sidebar snapshot (debounced) for the next cold start.
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      try {
        const slimProjects = projects.map(({ taskmaster: _taskmaster, ...rest }) => rest);
        localStorage.setItem(PROJECTS_CACHE_KEY, JSON.stringify(slimProjects));
      } catch {
        // Storage quota or unavailability — the cache is best-effort.
      }
    }, 500);
  ...
  // Cached rows already render instantly — only show the blocking loading
  // state when there is nothing on screen to revalidate behind.
  ...
  const handleSessionDelete = useCallback(
    (sessionIdToDelete: string) => {
      ...
      clearSessionPageCache(sessionIdToDelete);
