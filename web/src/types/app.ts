...
export type ProviderModelOption = {
  ...
  label: string;
  description?: string;
  /** Level-1 source group shown in two-level model pickers (vendor/upstream). */
  group?: string;
  recordId?: number;
  isCustom?: boolean;
  effort?: {
...
export type ProviderModelActions = {
  ...
  /** Hide (true) or re-show (false) a custom model without deleting it. */
  setHidden(provider: LLMProvider, existing: ProviderModelOption, hidden: boolean): Promise<void>;
};
