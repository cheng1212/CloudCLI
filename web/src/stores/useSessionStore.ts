 ...
 * The latest page of each opened session is mirrored into localStorage
 * (sessionMessageCache) as a best-effort display cache for fast PWA cold
 * starts. Backend JSONL remains the source of truth.
 */
...
async function refreshLatestSlotFromServer(
  ...
  saveSessionPageCache(sessionId, slot.serverMessages, slot.total, slot.hasMore);
...
export function useSessionStore() {
  ...
  /**
   * Seed an untouched slot with the locally cached latest page so the chat
   * pane renders instantly on cold start. The caller is expected to follow up
   * with `fetchFromServer` — server data then replaces the cached copy. A
   * slot that already holds fetched data is left untouched.
   */
  const hydrateFromCache = useCallback((sessionId: string): boolean => {
    const slot = getSlot(sessionId);
    if (slot.fetchedAt !== 0 || slot.serverMessages.length > 0) {
      return false;
    }
    const cached = loadSessionPageCache(sessionId);
  ...
  const fetchFromServer = useCallback(async (
    ...
    return enqueueHistoryMutation(slot, async () => {
      ...
      try {
        ...
        // Persist the fetched page so the next cold start can render this
        // session instantly. Only latest-page requests are cached; a full
        // "load all" transcript fetch (limit null) would bloat storage.
        if ((requestOptions.offset ?? 0) === 0 && requestOptions.limit !== null) {
          saveSessionPageCache(sessionId, data.messages, data.total, data.hasMore);
        }
