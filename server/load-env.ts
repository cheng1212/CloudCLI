...
// Isolation (cloudcli-custom): when CLOUDCLI_DATA_ROOT is set, the DB (and everything
// else) lands under that root instead of the shared ~/.cloudcli used by the real 3002.
import { getAuthDbPath, getCloudCliDataRoot } from './shared/data-root.js';

if (!process.env.DATABASE_PATH) {
  process.env.DATABASE_PATH = getAuthDbPath();
}
// Export the resolved root so data-root consumers share one source of truth at runtime.
process.env.CLOUDCLI_DATA_ROOT = getCloudCliDataRoot();
