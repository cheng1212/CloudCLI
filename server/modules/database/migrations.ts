...
/**
 * Adds the `isPinned` column used by the sidebar "conversations" view to keep
 * a user-pinned session at the top regardless of its updated_at timestamp.
 * Defaults to 0 (not pinned) so existing rows are unaffected.
 */
const addSessionPinnedColumn = (db: Database): void => {
  const sessionsTableInfo = getTableInfo(db, 'sessions');
  const columnNames = sessionsTableInfo.map((column) => column.name);
