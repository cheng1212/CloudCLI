...
const readCustomProviderModelRow = (
  ...
  const row = getConnection().prepare(`
    SELECT id, provider, model_id, model_name, sort_order, hidden
...
export const providerModelsDb = {
  ...
  /** Every row including hidden ones — model-library management surface. */
  listAllCustomProviderModels(provider: LLMProvider): CustomProviderModelRecord[] {
    const rows = getConnection().prepare(`
      SELECT id, provider, model_id, model_name, sort_order, hidden
      FROM provider_models
      WHERE provider = ?
  ...
  setCustomProviderModelHidden(
    ...
    const update = db.prepare(`
      ...
      WHERE provider = ? AND id = ?
    `).run(hidden ? 1 : 0, provider, recordId);
    if (update.changes === 0) {
      return null;
    }
