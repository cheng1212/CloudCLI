...
import { getCloudCliDataRoot } from '@/shared/data-root.js';
...
const PROFILE_ROOT = path.join(getCloudCliDataRoot(), 'browser-use', 'profiles');
...
export const browserUseService = {
  ...
  tokenMatches(candidate: string) {
    ...
    const b = createHash('sha256').update(getOrCreateMcpToken()).digest();
