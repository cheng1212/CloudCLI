...
async function runCliBootstrapFixture(isPlatform: boolean): Promise<BootstrapResult> {
  ...
  const fixtureSharedDirectory = path.join(fixtureServerDirectory, 'shared');
  const fixtureDatabasePath = path.join(fixtureRoot, 'auth.db');

  try {
    await mkdir(fixtureCliDirectory, { recursive: true });
    await mkdir(fixtureSharedDirectory, { recursive: true });
    await Promise.all([
      copyFile(
        path.join(applicationRoot, 'load-env.ts'),
        path.join(fixtureServerDirectory, 'load-env.ts'),
      ),
      copyFile(
        path.join(applicationRoot, 'modules', 'cli', 'cli.ts'),
        path.join(fixtureCliDirectory, 'cli.ts'),
      ),
      copyFile(
        path.join(applicationRoot, 'shared', 'data-root.ts'),
        path.join(fixtureSharedDirectory, 'data-root.ts'),
      ),
    ]);
    await writeFile(
...
import { createCliService } from ${JSON.stringify(sourceModuleUrl('modules', 'cli', 'cli.service.ts'))};
import { authenticateToken } from ${JSON.stringify(sourceModuleUrl('modules', 'auth', 'index.ts'))};
import { getBrowserUseRuntime } from ${JSON.stringify(sourceModuleUrl('modules', 'browser-use', 'index.ts'))};
import { closeConnection, initializeDatabase, userDb } from ${JSON.stringify(sourceModuleUrl('modules', 'database', 'index.ts'))};
