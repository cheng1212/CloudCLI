...
const createCatalogStore = () => {
  ...
  return {
    ...
    listCustomProviderModels(provider: LLMProvider) {
      return readRows(provider).filter((record) => !record.hidden);
    },
    listAllCustomProviderModels(provider: LLMProvider) {
      return [...readRows(provider)];
    },
    setCustomProviderModelHidden(provider: LLMProvider, recordId: number, hidden: boolean) {
      const record = readRows(provider).find((candidate) => candidate.recordId === recordId) ?? null;
      if (!record) {
        return null;
      }
