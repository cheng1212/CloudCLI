...
type ProviderModelsCatalogStore = Pick<
  ...
  | 'listAllCustomProviderModels'
  | 'setCustomProviderModelHidden'
...
/**
 * Custom model display names conventionally encode their transport source as
 * a "Source | Name" prefix (端口 / 直连 / Go / 代理). Model pickers group by
 * API source — where the request actually lands — so all OpenCode-gateway
 * models share one level-1 group even across vendor families, while vendor
 * APIs (DeepSeek / 智谱 / NVIDIA) each get their own group.
 */
export function deriveModelSourceGroup(modelId: string, displayName: string): string | undefined {
  const separatorIndex = displayName.indexOf(MODEL_SOURCE_LABEL_SEPARATOR);
  const prefix = separatorIndex > 0
    ? displayName.slice(0, separatorIndex).trim()
    : '';
  const label = separatorIndex > 0
    ? displayName.slice(separatorIndex + MODEL_SOURCE_LABEL_SEPARATOR.length).trim()
    : displayName;
  const id = modelId.toLowerCase();

  // OpenCode gateway: go-* ids, "Go ..." labels, or the Go transport prefix.
  if (
  ...
  // NVIDIA NIM models reach CloudCLI through the LiteLLM port/proxy blocks.
  if (
    prefix === '代理'
    || prefix === '端口'
    || /-nim$/.test(id)
    || /nvidia|nemotron|minimax/i.test(label)
  ) {
    return 'NVIDIA';
  }
...
const toCustomProviderModelOption = (
  ...
  const separatorIndex = record.model.indexOf(MODEL_SOURCE_LABEL_SEPARATOR);
  const hasSourcePrefix = separatorIndex > 0;
  const label = hasSourcePrefix
    ? record.model.slice(separatorIndex + MODEL_SOURCE_LABEL_SEPARATOR.length).trim()
    : record.model;
  const group = deriveModelSourceGroup(record.modelId, record.model);
...
export const createProviderModelsService = (dependencies: ProviderModelsServiceDependencies = {}) => {
  ...
  /**
   * Hidden rows stay persisted (routes/credentials untouched) but disappear
   * from every picker. Re-showing flips the flag without recreating anything.
   */
  const setCustomModelHidden = async (
    provider: LLMProvider,
    recordId: number,
    hidden: boolean,
  ): Promise<{ model: ProviderModelOption; models: ProviderModelsDefinition }> => {
    const predefined = await resolveProvider(provider).models.getSupportedModels();
    const updated = catalog.setCustomProviderModelHidden(provider, recordId, hidden);
    if (!updated) {
      throw new AppError('Model not found.', {
        code: 'MODEL_NOT_FOUND',
        statusCode: 404,
  ...
  /** Every persisted row including hidden ones, for the management surface. */
  const listAllCustomModels = (provider: LLMProvider): CustomProviderModelRecord[] =>
    catalog.listAllCustomProviderModels(provider);
