...
export const sessionsService = {
  ...
  /**
   * Archives or permanently deletes many persisted session rows in one call.
   *
   * Semantics mirror {@link deleteOrArchiveSessionById} per id: soft-delete
   * archives every found row, force-delete removes transcript files (where a
   * jsonl_path exists — OpenCode sessions intentionally have none) and then
   * the rows. Unknown ids are reported instead of failing the whole batch.
   */
  async deleteOrArchiveSessionsByIds(
    ...
    deleted: string[];
    notFound: string[];
    deletedFromDiskCount: number;
  }> {
    const uniqueIds = Array.from(new Set(sessionIds.map((id) => String(id ?? '').trim()).filter(Boolean)));
    const rows = sessionsDb.getSessionsByIds(uniqueIds);
    const foundIds = new Set(rows.map((row) => row.session_id));
    ...
    if (!options.force) {
      for (const row of rows) {
        ...
        archived.push(row.session_id);
      }
      return { archived, deleted, notFound, deletedFromDiskCount: 0 };
    }

    for (const row of rows) {
      if (options.deletedFromDisk && row.jsonl_path) {
        if (await removeFileIfExists(row.jsonl_path)) {
          deletedFromDiskCount += 1;
        }
      }
      deleted.push(row.session_id);
    }
    if (deleted.length > 0) {
      sessionsDb.deleteSessionsByIds(deleted);
    }

    return { archived, deleted, notFound, deletedFromDiskCount };
  },
  ...
  setSessionPinnedById(sessionId: string, isPinned: boolean): { sessionId: string; isPinned: boolean } {
    const session = sessionsDb.getSessionById(sessionId);
    if (!session) {
      throw new AppError(`Session "${sessionId}" was not found.`, {
        code: 'SESSION_NOT_FOUND',
    ...
    sessionsDb.setSessionPinned(sessionId, isPinned);
