...
test('normalizeOpenCodeModelId qualifies bare model ids before spawning', () => {
  const catalog = {
    OPTIONS: [
  ...
  // Already provider-qualified ids pass through untouched.
  assert.equal(normalizeOpenCodeModelId('opencode-go/glm-5.3-flash', catalog), 'opencode-go/glm-5.3-flash');
  // Bare ids that exist in the catalog get the catalog's full value.
  assert.equal(normalizeOpenCodeModelId('glm-5.3-flash', catalog), 'opencode-go/glm-5.3-flash');
  assert.equal(normalizeOpenCodeModelId('deepseek-v4-flash-vision-exp', catalog), 'opencode-go/deepseek-v4-flash-vision-exp');
  // Unknown bare ids still get the opencode-go prefix (CLI rejects bare ids with exit 1).
  assert.equal(normalizeOpenCodeModelId('some-future-model', catalog), 'opencode-go/some-future-model');
  assert.equal(normalizeOpenCodeModelId('some-future-model', null), 'opencode-go/some-future-model');
  // Empty/blank models stay untouched so the spawn skips the --model flag.
  assert.equal(normalizeOpenCodeModelId(undefined, catalog), undefined);
  assert.equal(normalizeOpenCodeModelId('  ', catalog), '  ');
});
