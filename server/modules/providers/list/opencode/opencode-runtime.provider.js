...
// OpenCode's `run --model` argument must be a provider-qualified id like
// `opencode-go/glm-5.3-flash`; a bare model id makes the CLI exit 1 with
// "Unexpected server error". Session-history readers (auth.db recorded
// selections) can still carry bare ids, so normalize before spawning: prefer
// a catalog match, then fall back to the `opencode-go` provider prefix.
function normalizeOpenCodeModelId(model, modelsDefinition) {
  ...
  const catalogMatch = options.find((option) => typeof option?.value === 'string' && option.value.endsWith(`/${trimmed}`));
  if (catalogMatch) {
    return catalogMatch.value;
  }
  return `opencode-go/${trimmed}`;
}
...
async function spawnOpenCode(command, options = {}, ws, context) {
  return new Promise((resolve, reject) => {
    ...
    void context.resolveResumeModel(sessionId, model).then(async (resolvedModel) => {
      ...
      const normalizedModel = normalizeOpenCodeModelId(resolvedModel, effortModels);
      const resolvedEffort = resolveOpenCodeEffort(normalizedModel, effort, effortModels);
