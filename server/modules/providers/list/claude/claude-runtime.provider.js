...
import { buildClaudeRouteSettings } from '@/shared/model-routes.js';
...
function mapCliOptionsToSDK(options = {}) {
  const { providerSessionId, cwd, toolsSettings, permissionMode, effort, routeSettings } = options;
  ...
  // whose model is the upstream model name. Claude Code loads settings layers by
  // priority — flag (from the SDK `settings` option / `--settings`) sits above
  // user, project, and local — so this payload's env block overrides whatever
  // endpoint the real ~/.claude/settings.json pins, without touching that file.
  // This is the only reliable injection point: Claude Code *ignores* env-var
  // overrides for its endpoint (empirically verified in the 3002 patch).
  if (routeSettings?.env?.ANTHROPIC_BASE_URL) {
    sdkOptions.settings = routeSettings;
    // Double-belt the route into the child env as well. Since SDK 0.2.113
    // `options.env` REPLACES process.env for the spawned CLI, so pinning the
    // route's endpoint/token here guarantees the child sees them even if the
    // current SDK version does not consume the `settings` object (which was
    // only empirically verified in the 3002 patch). The previous code deleted
    // these vars, which let the CLI fall back to the host env / user settings
    // (deepseek) when the `settings` payload was ignored.
    sdkOptions.env = {
      ...sdkOptions.env,
      ANTHROPIC_BASE_URL: routeSettings.env.ANTHROPIC_BASE_URL,
      ANTHROPIC_AUTH_TOKEN: routeSettings.env.ANTHROPIC_AUTH_TOKEN,
    };
    // A stray OpenAI-style key must not make the CLI pick a different provider
    // auth path than the routed token.
...
async function queryClaudeSDK(command, options = {}, ws, context) {
  ...
  try {
    ...
    // Unified routing: resolve the selected model id (session-scoped, else the
    // client's requested model) to an endpoint + token + upstream model name.
    // Only custom catalog ids route — pre-defined first-party models (default,
    // best, sonnet, ...) have no route and keep the CLI's configured endpoint,
    // so `defaultRoute` can never hijack the vendor's own models.
    const routeModel = resolvedModel || options.model;
    const routeCustomIds = new Set(
      (effortModels?.OPTIONS || []).filter((option) => option.isCustom).map((option) => option.value),
    );
    const routeSettings = routeCustomIds.has(routeModel) ? buildClaudeRouteSettings(routeModel) : null;

    // Diagnostic for routing: shows the exact model the gate evaluated and
    // whether a route (endpoint) was applied. Useful when the CLI lands on the
    // wrong provider and reports "selected model ... may not exist".
    if (process.env.CLOUDCLI_DEBUG_ROUTES === '1') {
      console.log('[Claude SDK] route resolution', {
        sessionId: sessionId || null,
        resolvedModel: resolvedModel || null,
        optionsModel: options.model || null,
        routeModel: routeModel || null,
        isCustom: routeCustomIds.has(routeModel),
        routeApplied: Boolean(routeSettings),
        endpoint: routeSettings?.env?.ANTHROPIC_BASE_URL || null,
        upstreamModel: routeSettings?.model || null,
      });
    }

    // Surface the model that will actually serve this run. The route is the
    // endpoint truth: direct models keep their name, everything else goes to
    // the default route (LiteLLM 4000) under the catalog model id. The client
    // shows this instead of the DB model id so "which model is working" always
    // matches where the request really lands.
    if (ws?.readyState === 1 || typeof ws?.send === 'function') {
      ws.send(createNormalizedMessage({
        kind: 'status',
        text: 'model_routed',
        model: routeSettings?.model || routeModel || '',
        endpoint: routeSettings?.env?.ANTHROPIC_BASE_URL || null,
