...
const parseSessionPinPayload = (payload: unknown): boolean => {
  if (!payload || typeof payload !== 'object') {
    throw new AppError('Request body must be an object.', {
      code: 'INVALID_REQUEST_BODY',
      statusCode: 400,
  ...
  if (typeof body.pinned !== 'boolean') {
    throw new AppError('"pinned" must be a boolean.', {
      code: 'INVALID_SESSION_PIN',
      statusCode: 400,
    });
  }
...
/**
 * Lists every persisted custom-model row including hidden ones. The model
 * library uses this to offer "show again" for rows hidden from the pickers.
 */
router.get(
  '/:provider/models/hidden',
  asyncHandler(async (req: Request, res: Response) => {
    const provider = parseProvider(req.params.provider);
    const records = providerModelsService.listAllCustomModels(provider);
    res.json(createApiSuccessResponse({ provider, records }));
  }),
);

/**
 * Hides or re-shows one custom model without deleting it: routes and stored
 * sessions stay intact, the row just leaves or rejoins the model pickers.
 */
router.patch(
  '/:provider/models/:recordId/hidden',
  asyncHandler(async (req: Request, res: Response) => {
    const provider = parseProvider(req.params.provider);
    const recordId = parseModelRecordId(req.params.recordId);
    const hidden = Boolean((req.body as { hidden?: unknown } | null)?.hidden);
    const result = await providerModelsService.setCustomModelHidden(provider, recordId, hidden);
...
// Batch archive/delete for the sidebar bulk-manage mode. POST (not DELETE)
// because the id list lives in a JSON body; per-id semantics are identical to
// the single DELETE route, and unknown ids are reported instead of failing.
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const sessionIds = rawIds
      .filter((id): id is string => typeof id === 'string' && SESSION_ID_PATTERN.test(id.trim()))
      .map((id) => id.trim());
    if (sessionIds.length === 0) {
      throw new AppError('sessionIds must be a non-empty array of valid session ids.', {
        code: 'INVALID_SESSION_IDS',
        statusCode: 400,
      });
    }
    if (sessionIds.length > 200) {
      throw new AppError('sessionIds exceeds the maximum batch size of 200.', {
        code: 'BATCH_TOO_LARGE',
        statusCode: 400,
      });
    }
    const force = typeof body.force === 'boolean' ? body.force : false;
    const deletedFromDisk = typeof body.deletedFromDisk === 'boolean' ? body.deletedFromDisk : force;
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const sessionId = parseSessionId(req.params.sessionId);
    const pinned = parseSessionPinPayload(req.body);
    const result = sessionsService.setSessionPinnedById(sessionId, pinned);
    res.json(createApiSuccessResponse(result));
  }),
);
