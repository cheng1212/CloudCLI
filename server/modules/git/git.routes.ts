...
async function resolveRepositoryFilePath(projectPath, filePath) {
  // Containment used to be dead code here: validateFilePath only enforces its
  // traversal branch when a root is supplied. Files may legitimately live
  // anywhere inside the repository (not just below projectPath), so the
  // boundary is the repo root, resolved before the check runs.
  const repositoryRootPath = await getRepositoryRootPath(projectPath);
  validateFilePath(filePath, repositoryRootPath);
