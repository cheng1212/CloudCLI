...
const defaultDependencies: CloneProjectDependencies = {
  ...
  spawnGitClone: (cloneUrl: string, clonePath: string, githubToken?: string): GitCloneProcess =>
    spawn('git', ['clone', '--progress', '--', cloneUrl, clonePath], {
      stdio: ['ignore', 'pipe', 'pipe'],
      env: {
        ...process.env,
        GIT_TERMINAL_PROMPT: '0',
        // Credentials travel through the env-fed credential helper so the
        // token never appears in process argv (visible via ps/task manager).
        ...(githubToken
          ? {
              GIT_CONFIG_COUNT: '1',
...
export async function startCloneProject(
  ...
  // The token is handed to git through the env credential helper inside
  // spawnGitClone — never through the URL, where it would land in argv.
  const cloneUrl = normalizedGithubUrl;
  ...
  gitProcess.stdout?.on('data', (data: Buffer | string) => {
    const message = sanitizeGitError(data.toString().trim(), githubToken);
    if (message) {
