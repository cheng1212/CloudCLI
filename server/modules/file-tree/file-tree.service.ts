...
async function resolvePathInsideProject(
  projectRoot: string,
  targetPath: string,
  ...
  // Lexical containment misses symlinks planted inside the project (a cloned
  // repo can commit one and redirect reads/writes/deletes anywhere). Re-check
  // the realpath of the deepest existing ancestor before handing the path out.
  if (realpath) {
    ...
    const effectiveRoot = realAncestor ?? (await realpath(projectRoot));
    if (effectiveRoot !== projectRoot && !effectiveRoot.startsWith(normalizedProjectRoot)) {
      throw createFileTreeError('Path must be under project root', 403, 'PATH_OUTSIDE_PROJECT');
    }
  }
...
export function createFileTreeService(dependencies: FileTreeServiceDependencies): FileTreeServices {
  ...
  return {
    ...
    async readTextFile(projectId, filePath) {
      ...
      const resolvedPath = await resolvePathInsideProject(projectRoot, filePath, fileSystem.realpath);
      try {
    ...
    async createEntry(input) {
      ...
      const targetPath = input.parentPath
        ...
        : input.name;
      const resolvedPath = await resolvePathInsideProject(projectRoot, targetPath, fileSystem.realpath);
    ...
    async renameEntry(input) {
      ...
      const projectRoot = await resolveProjectRoot(input.projectId);
      const resolvedOldPath = await resolvePathInsideProject(projectRoot, input.oldPath, fileSystem.realpath);
      ...
      await resolvePathInsideProject(projectRoot, resolvedNewPath, fileSystem.realpath);
      try {
    ...
    async deleteEntry(input) {
      const projectRoot = await resolveProjectRoot(input.projectId);
      const resolvedPath = await resolvePathInsideProject(projectRoot, input.targetPath, fileSystem.realpath);
      let stats;
      try {
    ...
    async storeUploadedFiles(input) {
      ...
      try {
        ...
        const resolvedTargetDirectory = !input.targetPath
          ...
          : await resolvePathInsideProject(projectRoot, input.targetPath, fileSystem.realpath);
        ...
        for (let fileIndex = 0; fileIndex < input.files.length; fileIndex += 1) {
          ...
          try {
            await resolvePathInsideProject(projectRoot, destinationPath, fileSystem.realpath);
          } catch (error) {
