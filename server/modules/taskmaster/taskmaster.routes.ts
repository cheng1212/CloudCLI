...
export function createTaskmasterRouter(dependencies: TaskmasterRouterDependencies): express.Router {
     ...
     */
    // PRD filenames live inside .taskmaster/docs and must never traverse
    // out of it. One shared whitelist for every endpoint that takes a
    // client-supplied PRD fileName (GET, POST, parse-prd, apply-template).
    const validatePrdFileName = (fileName) => {
        if (!fileName || !fileName.match(/^[\w\-. ]+\.(txt|md)$/)) {
            return false;
        }
        return true;
    };
    ...
    router.get('/prd/:projectId/:fileName', async (req, res) => {
        try {
            ...
            if (!validatePrdFileName(fileName)) {
                return res.status(400).json({
                    error: 'Invalid filename',
                    message: 'Filename must end with .txt or .md and contain only alphanumeric characters, spaces, dots, and dashes'
                });
            ...
            if (!path.resolve(filePath).startsWith(path.resolve(docsDir) + path.sep)) {
                return res.status(400).json({
                    error: 'Invalid filename',
                    message: 'Filename must not traverse outside .taskmaster/docs'
                });
            }
    ...
    router.post('/parse-prd/:projectId', async (req, res) => {
        try {
            ...
            if (!path.resolve(prdPath).startsWith(path.resolve(projectPath, '.taskmaster', 'docs') + path.sep)) {
                return res.status(400).json({
                    error: 'Invalid filename',
                    message: 'Filename must not traverse outside .taskmaster/docs'
                });
